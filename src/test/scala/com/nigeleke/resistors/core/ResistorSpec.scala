...
class ResistorSpec extends WordSpec with Matchers {
  ...
  "Resistors are defined in series" in {
    val s3 = Resistor.setFor(Series.E3)
    s3.map(_.baseValue) should contain theSameElementsAs(Seq(10, 22, 47))

    val s6 = Resistor.setFor(Series.E6)
    s6.map(_.baseValue) should contain theSameElementsAs(Seq(10, 15, 22, 33, 47, 68))

    val s12 = Resistor.setFor(Series.E12)
    s12.map(_.baseValue) should contain theSameElementsAs(Seq(10, 12, 15, 18, 22, 27, 33, 39, 47, 56, 68, 82))

    val s24 = Resistor.setFor(Series.E24)
    s24.map(_.baseValue) should contain theSameElementsAs(Seq(
      10, 11, 12, 13, 15, 16, 18, 20, 22, 24, 27, 30, 33, 36, 39, 43, 47, 51, 56, 62, 68, 75, 82, 91))

    val s48 = Resistor.setFor(Series.E48)
    s48.map(_.baseValue) should contain theSameElementsAs(Seq(
    ...
    s96.map(_.baseValue) should contain theSameElementsAs(Seq(
    ...
    s192.map(_.baseValue) should contain theSameElementsAs(Seq(
  ...
  "Resistors series are defined by base values" in {
    Resistor.fullSet.map(_.baseValue).size should be(216)
  }

  "Resistor multipliers will include milli to giga" in {
  ...
  "A set of Resistors will be the base values with all multipliers" in {
    ...
    val resistors = Resistor.setFor(Series.E24)

    val baseValues = series.baseValues.asScala.map(_.toDouble)
    resistors.size should be (baseValues.size * Resistor.multiples.size)

    val values = resistors.map(_.value)
    values should contain allElementsOf(baseValues.map(Ohms(_) * 1e-2))
    values should contain allElementsOf(baseValues.map(Ohms(_) * 1e-1))
    values should contain allElementsOf(baseValues.map(Ohms(_) * 1e0))
    values should contain allElementsOf(baseValues.map(Ohms(_) * 1e1))
    values should contain allElementsOf(baseValues.map(Ohms(_) * 1e2))
    values should contain allElementsOf(baseValues.map(Ohms(_) * 1e3))
    values should contain allElementsOf(baseValues.map(Ohms(_) * 1e4))
    values should contain allElementsOf(baseValues.map(Ohms(_) * 1e5))
    values should contain allElementsOf(baseValues.map(Ohms(_) * 1e6))
    values should contain allElementsOf(baseValues.map(Ohms(_) * 1e7))
    values should contain allElementsOf(baseValues.map(Ohms(_) * 1e8))
    values should contain allElementsOf(baseValues.map(Ohms(_) * 1e9))
  }
