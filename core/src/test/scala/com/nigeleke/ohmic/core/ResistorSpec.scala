package com.nigeleke.ohmic.core
...
class ResistorSpec extends AnyWordSpec with Matchers:
  ...
  "Resistor base values have multipliers which should contain powers of ten from -2 (millis) to 9 (gigs) inclusive" in {
    Multiplier.values.map(
      _.apply(1)
    ) should contain allOf (1e-2, 1e-1, 1e0, 1e1, 1e2, 1e3, 1e4, 1e5, 1e6, 1e7, 1e8, 1e9)
  }
  ...
  "A set of Resistors will be the base values with all multipliers" in {
    ...
    resistors.size should be(series.size * multipliers.size)

    val expectedValues = (for {
      baseValue  <- series
      multiplier <- multipliers
    } yield Ohms(multiplier(baseValue))).toSet
  ...
  "A Resistor will be colour coded for base value & multiplier; we ignore tolerance" in {
    resistorWithValue(Ohms(22)).colourCodes should contain theSameElementsInOrderAs (Seq(
      BandColour.Red,
      BandColour.Red,
      BandColour.Black
    ))

    resistorWithValue(Ohms(220)).colourCodes should contain theSameElementsInOrderAs (Seq(
      BandColour.Red,
      BandColour.Red,
      BandColour.Brown
    ))

    resistorWithValue(Ohms(2200)).colourCodes should contain theSameElementsInOrderAs (Seq(
      BandColour.Red,
      BandColour.Red,
      BandColour.Red
    ))
